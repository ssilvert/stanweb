import KcAdminClient from "../../../web_modules/keycloak-admin.js";
...
const keycloakAuthUrl = () => {
  ...
  const searchParams = new URLSearchParams(window.location.search); // passed in as query param

  const authUrlFromParam = searchParams.get("keycloak-server");
  if (authUrlFromParam) return authUrlFromParam + authContext; // dev mode

  if (!window.location.pathname.startsWith("/adminv2")) return "http://localhost:8180" + authContext; // demo mode

  if (searchParams.get("demo")) return "http://localhost:8080" + authContext; // admin console served from keycloak server
